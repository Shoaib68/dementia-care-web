...
async function handlePATCH(
  ...
  // Deactivate the patient's user account
  ...
  if (patientUserError) {
    throw new ApiError(
      `Failed to deactivate patient: ${patientUserError.message}`,
      500,
      'PATIENT_DEACTIVATE_FAILED'
    );
  }
  ...
  if (assignment?.caregiver_id) {
    ...
    if (caregiverUserError) {
      throw new ApiError(
        `Failed to deactivate caregiver: ${caregiverUserError.message}`,
        500,
        'CAREGIVER_DEACTIVATE_FAILED'
      );
    }
  }

  return createSuccessResponse(
    { success: true, patientId, caregiverDeactivated: !!assignment?.caregiver_id },
    'Patient and caregiver deactivated successfully'
  );
}
